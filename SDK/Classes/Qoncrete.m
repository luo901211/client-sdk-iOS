...
//  QoncreteDemo
//
//  Created by luo on 2016/12/6.
//  Copyright © 2016年 Goyoo. All rights reserved.
//

#import "Qoncrete.h"

#define HOST @"https://qoncrete.com"
...
- (void)endConfig {
    ...
    self.sendLogEndpoint = [NSString stringWithFormat:@"http%@://log.qoncrete.com/%@?token=%@",self.secureTransport ? @"s": @"", self.sourceID,self.apiToken];
    self.sendBatchLogEndpoint = [NSString stringWithFormat:@"http%@://log.qoncrete.com/%@/batch?token=%@",self.secureTransport ? @"s": @"", self.sourceID,self.apiToken];
    self.keepAliveAgent = @{
                            @"keepAlive": @YES,
                            @"keepAliveMsec": @5000,
